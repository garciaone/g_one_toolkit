...
def mesh_soft_falloff_mask(frame_shape, landmarks, seed_indices,
    ...
    """
    Mesh-aware soft falloff mask in [0,1] following MediaPipe FaceMesh topology.

    Args:
        frame_shape: (H, W, C) of the target image.
        landmarks:   iterable of NormalizedLandmark (one face).
        seed_indices:list[int] seed vertices (region center/outline).
        inner:       fully-white core radius (in chosen distance units).
        feather:     soft roll-off width to 0 outside core.
        space:       "3d" -> use (x,y,z) normalized coords for edge lengths (pose-robust)
                     "pixel" -> use (x*W, y*H) pixel coords for edge lengths

    Returns:
        mask: float32 array HxW in [0,1]
    """
    H, W = frame_shape[:2]
            ...
            for face in result.face_landmarks:
                ...
                if falloff_mode == "simple-2D":
                    ...
                    if optional_selection_set != None or mask_selection_set != None:
                        tint  = np.array([0, 255, 255], dtype=frame_bgr_f32.dtype)      # BGR
                ...
                if visualization_mode:
                    ...
                    solutions.drawing_utils.draw_landmarks(
                        ...
                        connection_drawing_spec=mp_drawing.DrawingSpec(
                            color=(0, 200, 200),                   # BGR colour
                            thickness=1,                           # line thickness
                            circle_radius=0                        # unused for lines
                        )
                    )

                    draw_landmark_indices(frame_bgr, face, subset=None, stride=1,color=(255,0,255),dot_radius=1,font_scale=.2, thickness=1)
