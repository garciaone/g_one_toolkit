...
def trackFaceMasks(images,**kargs):
    ...
    for frame_idx, frame in enumerate(frames):
        ...
        if result.face_landmarks:
            for face in result.face_landmarks:
                mask = hard_mask_from_indices(frame_bgr.shape, face, IDX_R_EYE)

                soft_mask = soften_mask(mask, inner_px=0, feather_px=20)

                mask01 = soft_mask.astype(np.float32)
                mask3  = mask01[..., None] 
                imgf   = frame_bgr.astype(np.float32)

                # out = (imgf * mask3).astype(frame_bgr.dtype)
                # out = (imgf * (1.0 - mask3)).astype(frame_bgr.dtype)
                # out = (imgf + mask3).astype(frame_bgr.dtype)

                tint  = np.array([0, 255, 255], dtype=imgf.dtype)      # BGR
                alpha = 0.6                                            # blend strength
                out = imgf * (1 - alpha*mask3) + tint * (alpha*mask3)
                out = out.astype(frame_bgr.dtype)
